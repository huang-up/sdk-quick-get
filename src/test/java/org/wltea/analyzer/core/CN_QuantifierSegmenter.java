 ...
 * 中文数量词子分词器
 */
class CN_QuantifierSegmenter implements org.wltea.analyzer.core.ISegmenter {
	 ...
	 * 处理数词
	 */
	private void processCNumber(org.wltea.analyzer.core.AnalyzeContext context){
		if(nStart == -1 && nEnd == -1){//初始状态
			if(org.wltea.analyzer.core.CharacterUtil.CHAR_CHINESE == context.getCurrentCharType()
	 ...
	 * @param context
	 */
	private void processCount(org.wltea.analyzer.core.AnalyzeContext context){
		// 判断是否需要启动量词扫描
		...
		if(org.wltea.analyzer.core.CharacterUtil.CHAR_CHINESE == context.getCurrentCharType()){
			...
			if(!this.countHits.isEmpty()){
				...
				for(Hit hit : tmpArray){
					...
					if(hit.isMatch()){
						//输出当前的词
						org.wltea.analyzer.core.Lexeme newLexeme = new org.wltea.analyzer.core.Lexeme(context.getBufferOffset() , hit.getBegin() , context.getCursor() - hit.getBegin() + 1 , org.wltea.analyzer.core.Lexeme.TYPE_COUNT);
			...
			if(singleCharHit.isMatch()){//首字成量词词
				//输出当前的词
				org.wltea.analyzer.core.Lexeme newLexeme = new org.wltea.analyzer.core.Lexeme(context.getBufferOffset() , context.getCursor() , 1 , org.wltea.analyzer.core.Lexeme.TYPE_COUNT);
				context.addLexeme(newLexeme);
	 ...
	 * @return
	 */
	private boolean needCountScan(org.wltea.analyzer.core.AnalyzeContext context){
		if((nStart != -1 && nEnd != -1 ) || !countHits.isEmpty()){
			...
			if(!context.getOrgLexemes().isEmpty()){
				org.wltea.analyzer.core.Lexeme l = context.getOrgLexemes().peekLast();
				if((org.wltea.analyzer.core.Lexeme.TYPE_CNUM == l.getLexemeType() ||  org.wltea.analyzer.core.Lexeme.TYPE_ARABIC == l.getLexemeType())
	 ...
	 * @param context
	 */
	private void outputNumLexeme(org.wltea.analyzer.core.AnalyzeContext context){
		if(nStart > -1 && nEnd > -1){
			//输出数词
			org.wltea.analyzer.core.Lexeme newLexeme = new org.wltea.analyzer.core.Lexeme(context.getBufferOffset() , nStart , nEnd - nStart + 1 , Lexeme.TYPE_CNUM);
			context.addLexeme(newLexeme);
