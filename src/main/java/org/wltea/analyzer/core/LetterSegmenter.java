 ...
 * 英文字符及阿拉伯数字子分词器
 */
class LetterSegmenter implements org.wltea.analyzer.core.ISegmenter {
...
//	 * @param input
	 ...
	 * @return
	 */
	private boolean processMixLetter(org.wltea.analyzer.core.AnalyzeContext context){
		boolean needLock = false;
		
		if(this.start == -1){//当前的分词器尚未开始处理字符
			if(org.wltea.analyzer.core.CharacterUtil.CHAR_ARABIC == context.getCurrentCharType()
					|| org.wltea.analyzer.core.CharacterUtil.CHAR_ENGLISH == context.getCurrentCharType()){
			...
			}else if(org.wltea.analyzer.core.CharacterUtil.CHAR_USELESS == context.getCurrentCharType()
						&& this.isLetterConnector(context.getCurrentChar())){
				//记录下可能的结束位置
				this.end = context.getCursor();
			}else{
				//遇到非Letter字符，输出词元
				org.wltea.analyzer.core.Lexeme newLexeme = new org.wltea.analyzer.core.Lexeme(context.getBufferOffset() , this.start , this.end - this.start + 1 , org.wltea.analyzer.core.Lexeme.TYPE_LETTER);
	 ...
	 * @return
	 */
	private boolean processEnglishLetter(org.wltea.analyzer.core.AnalyzeContext context){
		boolean needLock = false;
		
		if(this.englishStart == -1){//当前的分词器尚未开始处理英文字符	
			if(org.wltea.analyzer.core.CharacterUtil.CHAR_ENGLISH == context.getCurrentCharType()){
				//记录起始指针的位置,标明分词器进入处理状态
			...
			if(org.wltea.analyzer.core.CharacterUtil.CHAR_ENGLISH == context.getCurrentCharType()){
				...
				//遇到非English字符,输出词元
				org.wltea.analyzer.core.Lexeme newLexeme = new org.wltea.analyzer.core.Lexeme(context.getBufferOffset() , this.englishStart , this.englishEnd - this.englishStart + 1 , org.wltea.analyzer.core.Lexeme.TYPE_ENGLISH);
				context.addLexeme(newLexeme);
				this.englishStart = -1;
				this.englishEnd= -1;
			}
	 ...
	 * @return
	 */
	private boolean processArabicLetter(org.wltea.analyzer.core.AnalyzeContext context){
		boolean needLock = false;
		...
		if(this.arabicStart == -1){//当前的分词器尚未开始处理数字字符	
			...
			if(org.wltea.analyzer.core.CharacterUtil.CHAR_ARABIC == context.getCurrentCharType()){
				...
				////遇到非Arabic字符,输出词元
				org.wltea.analyzer.core.Lexeme newLexeme = new org.wltea.analyzer.core.Lexeme(context.getBufferOffset() , this.arabicStart , this.arabicEnd - this.arabicStart + 1 , org.wltea.analyzer.core.Lexeme.TYPE_ARABIC);
				context.addLexeme(newLexeme);
				this.arabicStart = -1;
				this.arabicEnd = -1;
			}
		...
		if(context.isBufferConsumed() && (this.arabicStart != -1 && this.arabicEnd != -1)){
            //生成已切分的词元
            org.wltea.analyzer.core.Lexeme newLexeme = new org.wltea.analyzer.core.Lexeme(context.getBufferOffset() ,  this.arabicStart , this.arabicEnd - this.arabicStart + 1 , Lexeme.TYPE_ARABIC);
            context.addLexeme(newLexeme);
            this.arabicStart = -1;
            this.arabicEnd = -1;
		}
