...
class LexemePath extends QuickSortSet implements Comparable<LexemePath>{
	 ...
	 * @return 
	 */
	boolean addCrossLexeme(org.wltea.analyzer.core.Lexeme lexeme){
		if(this.isEmpty()){
	 ...
	 * @return 
	 */
	boolean addNotCrossLexeme(org.wltea.analyzer.core.Lexeme lexeme){
		if(this.isEmpty()){
			this.addLexeme(lexeme);
			this.pathBegin = lexeme.getBegin();
			this.pathEnd = lexeme.getBegin() + lexeme.getLength();
			...
			this.addLexeme(lexeme);
			this.payloadLength += lexeme.getLength();
			org.wltea.analyzer.core.Lexeme head = this.peekFirst();
			this.pathBegin = head.getBegin();
			org.wltea.analyzer.core.Lexeme tail = this.peekLast();
	 ...
	 * @return
	 */
	org.wltea.analyzer.core.Lexeme removeTail(){
		org.wltea.analyzer.core.Lexeme tail = this.pollLast();
		if(this.isEmpty()){
			this.pathBegin = -1;
			this.pathEnd = -1;
			this.payloadLength = 0;			
			...
			org.wltea.analyzer.core.Lexeme newTail = this.peekLast();
	 ...
	 * @return
	 */
	boolean checkCross(org.wltea.analyzer.core.Lexeme lexeme){
